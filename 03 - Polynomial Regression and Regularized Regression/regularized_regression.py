...
class RegularizedRegression(object):
    ...
    def poly_feature_transform(self,X,poly_order=1):
        ...
        for i in range(2, poly_order+1):
            nth_poly = np.power(X, i)
            f_transform = np.concatenate((f_transform, nth_poly), axis=1)
        ...
        f_transform = np.concatenate((f_transform, np.ones((X.shape[0], 1))), axis=1)
    ...
    def train_analytic(self, X, y, poly_order=1, lambda_reg=0):
        ...
        f_transform = self.poly_feature_transform(X, poly_order)
        f_transformT = f_transform.T
        weight = np.add(f_transformT@f_transform, np.multiply(lambda_reg, np.identity(f_transformT.shape[0])))
        weight = np.linalg.pinv(weight)
        weight = weight@f_transformT@y
        self.params['W'] = weight
    ...
    def loss(self, X, y=None, lambda_reg=0.0):
        ...
        diff = prediction-y
        loss = ((diff.T@diff)/(2*len(y))).flatten()[0]
        ...
        l2_grad = lambda_reg * W
        grads['W'] = ((diff*X).sum(axis=0)/N + l2_grad.flatten()).reshape(D,1)
