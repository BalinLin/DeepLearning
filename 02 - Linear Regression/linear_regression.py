...
class LinearRegression(object):
    ...
    def feature_transform(self, X):
        ...
        for i in range(X.shape[0]):
            X_transformed[i] = np.append(X[i],1.0)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def train_analytic(self, X, y):
        ...
        weight = np.linalg.pinv(X.T@X)@X.T@y
    ...
    def loss(self, X, y=None):
        ...
        grads['W'] = ((diff*X).sum(axis=0)/N).reshape(D,1)
    ...
    def train(self, X, y, X_val=None, y_val=None,learning_rate=1e-3, learning_rate_decay=0.95,
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            randomIndex = np.random.choice(num_train, batch_size, replace=False)
            X_batch = np.take(X,randomIndex,axis=0)
            y_batch = np.take(y,randomIndex,axis=0)
            #########################################################################
            #                             END OF YOUR CODE                          #
            #########################################################################
